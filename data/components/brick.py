...
from ..types import Vector2, Game_Object, State_Machine, State, Entity, Rectangle
from .. import sprites
from .. import sounds
...
class Question(Game_Object):
    ...
    class Animation():
        ...
        def closed_anim(self):
            ...
            self.outer_timer += c.var_time
            if self.outer_timer > 20 * c.var_time:
                self.inner_timer += c.var_time
                if self.inner_timer > 6 * c.var_time:
                    self.closed_frame += 1
                    self.inner_timer = 0
...
class Brick_Fragment(Entity):
    ...
    class Animation():
        ...
        def anim(self):
            ...
            if self.anim_timer > 8 * c.var_time:
                self.anim_frame += 1
                self.anim_timer = 0
